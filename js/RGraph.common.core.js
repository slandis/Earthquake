...
{
var context = canvas.getContext('2d');context.fillStyle = arguments[1] ? String(arguments[1]) : 'white';context = canvas.getContext('2d');context.beginPath();context.fillRect(-5,-5,canvas.width + 5,canvas.height + 5);context.fill();if(RGraph.ClearAnnotations){
...
context.beginPath();context.fillStyle = '#666';if(obj.Get('chart.key.rounded')){        
RGraph.NoShadow(obj);context.beginPath();RGraph.filledCurvyRect(context,
xpos + obj.Get('chart.key.shadow.offsetx'),
gutter + 5 + obj.Get('chart.key.shadow.offsety'),
width - 5,
5 + ( (textsize + 5) * key.length),
5);context.closePath();context.fill();} else {
context.fillRect(xpos + 2, gutter + 5 + 2, width - 5, 5 + ( (textsize + 5) * key.length));}
context.fill();context.fillStyle = obj.Get('chart.key.background');}
if(obj.Get('chart.key.rounded') == true){
context.beginPath();context.strokeStyle = '#333';RGraph.strokedCurvyRect(context, xpos, gutter + 5, width - 5, 5 + ( (textsize + 5) * key.length),4);context.closePath();context.stroke();RGraph.NoShadow(obj);context.beginPath();RGraph.filledCurvyRect(context, xpos, gutter + 5, width - 5, 5 + ( (textsize + 5) * key.length), 5);context.closePath();context.fill();} else {
...
context.beginPath();context.shadowColor = 'rgba(0,0,0,0)';for (var i=key.length - 1; i>=0; i--){
var j = Number(i) + 1;context.fillStyle = colors[i];context.fillRect(xpos + 5, 5 + gutter + (5 * j) + (textsize * j) - (textsize), textsize, textsize);context.stroke();context.fill();context.fillStyle = obj.Get('chart.text.color');RGraph.Text(context,obj.Get('chart.text.font'),textsize,xpos + 21,gutter + (5 * j) + (textsize * j) + 4,key[i]);}
} else {
...
if(obj.type == 'scatter'){
var xCoord = (((x - obj.Get('chart.gutter')) / (obj.canvas.width - (2 * obj.Get('chart.gutter')))) * (obj.Get('chart.xmax') - obj.Get('chart.xmin'))) + obj.Get('chart.xmin');xCoord = xCoord.toFixed(obj.Get('chart.scale.decimals'));var yCoord = obj.max - (((y - obj.Get('chart.gutter')) / (obj.canvas.height - (2 * obj.Get('chart.gutter')))) * obj.max);yCoord = yCoord.toFixed(obj.Get('chart.scale.decimals'));var div = RGraph.Registry.Get('chart.coordinates.coords.div');var mouseCoords = RGraph.getMouseXY(e);var canvasXY = RGraph.getCanvasXY(canvas);if(!div){
...
context.fillRect(x + r, y, w - r - r, h);context.fillRect(x, y + r, r, h - r - r);context.fillRect(x + w - r, y + r, r, h - r - r);context.fill();}
